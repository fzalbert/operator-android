...
object MockRepository {
    ...
    val operationDefinitions: List<OperationDefinition> = listOf(
        ...
        OperationDefinition(OperationType.NEST_CONTROL, TargetType.CAGE, false, "Сохранить контроль", listOf(
            OperationField("fed", "Сытых", FieldType.NUMBER, true), OperationField("hungry", "Голодных", FieldType.NUMBER, true), OperationField("dead", "Мертвых", FieldType.NUMBER, true), OperationField("nestState", "Состояние гнезда", FieldType.SELECT, true, options = listOf("Норма", "Мокрое", "Мало стружки", "Нужно вмешательство"))
        ), listOf(RoleId.OPERATOR), true),
        ...
        OperationDefinition(OperationType.ANIMAL_TRANSFER, TargetType.CAGE, false, "Перевести", listOf(OperationField("fromCage", "Из клетки", FieldType.SELECT, true, options = listOf("Выберите клетку") + hangarACageCodes), OperationField("toCage", "В клетку", FieldType.SELECT, true, options = listOf("Выберите клетку") + hangarACageCodes), OperationField("animalCount", "Количество животных", FieldType.NUMBER, true)), listOf(RoleId.OPERATOR, RoleId.GENERAL_WORKER)),
        OperationDefinition(OperationType.ANIMAL_SETTLEMENT, TargetType.RABBIT, true, "Заселить", listOf(OperationField("rfid", "RFID животного", FieldType.TEXT, true), OperationField("toCage", "Клетка назначения", FieldType.SELECT, true, options = listOf("Выберите клетку") + hangarACageCodes)), listOf(RoleId.OPERATOR, RoleId.GENERAL_WORKER)),
        OperationDefinition(OperationType.OKROL, TargetType.CAGE, false, "Окрол учтен", listOf(OperationField("bornAlive", "Живых", FieldType.NUMBER, true), OperationField("bornDead", "Мертвых", FieldType.NUMBER, true)), listOf(RoleId.OPERATOR), true),
        OperationDefinition(OperationType.LACTATION_CONTROL, TargetType.CAGE, false, "Лактация проверена", listOf(OperationField("status", "Статус", FieldType.SELECT, true, options = listOf("Норма", "Недостаточно молока", "Нужна подсадка", "Нужен технолог"))), listOf(RoleId.OPERATOR, RoleId.CHIEF_TECHNOLOGIST)),
        ...
        OperationDefinition(OperationType.ANIMAL_DEPARTURE, TargetType.RABBIT, true, "Выбытие учтено", listOf(OperationField("rfid", "RFID животного", FieldType.TEXT, true), OperationField("reason", "Причина", FieldType.SELECT, true, options = listOf("Падеж", "Выбраковка", "Перемещение")), OperationField("comment", "Комментарий", FieldType.TEXT)), listOf(RoleId.OPERATOR, RoleId.CHIEF_TECHNOLOGIST)),
    ...
    fun initialTasks(): List<MobileTask> = listOf(
        ...
        MobileTask("task-13", "Заселение животных", OperationType.ANIMAL_SETTLEMENT, "ws-1", "h-1", "emp-1", "2026-07-09", "17:10", 90, Priority.NORMAL, TaskStatus.NEW, rabbitChecklist("settlement", 6), true, RoleId.CHIEF_TECHNOLOGIST, AcceptanceStatus.NOT_REQUIRED),
        MobileTask("task-14", "Выбытие животных", OperationType.ANIMAL_DEPARTURE, "ws-1", "h-1", "emp-1", "2026-07-09", "17:30", 60, Priority.NORMAL, TaskStatus.NEW, rabbitChecklist("departure", 6), false),
        MobileTask("task-15", "Завоз самок", OperationType.FEMALE_DELIVERY, "ws-1", "h-1", "emp-1", "2026-07-09", "18:00", 120, Priority.NORMAL, TaskStatus.NEW, rabbitChecklist("delivery", 6), true, RoleId.CHIEF_TECHNOLOGIST, AcceptanceStatus.NOT_REQUIRED),
    )
}
